...
#ifndef OV_EVAL_RECORDER_ROS2_H
#define OV_EVAL_RECORDER_ROS2_H
...
/**
 * @brief This class takes in published poses (ROS2) and writes them to file.
 *
 * This is the ROS2 version of the Recorder class.
...
public:
   ...
   */
  RecorderROS2(std::string filename) {
    ...
    if (!parent.empty() && parent.string() != ".") {
      if (boost::filesystem::create_directories(parent)) {
        RCLCPP_INFO(rclcpp::get_logger("RecorderROS2"), "Created folder path to output file: %s", parent.c_str());
      }
    }
    // If it exists, then delete it
    if (boost::filesystem::exists(filename)) {
      RCLCPP_WARN(rclcpp::get_logger("RecorderROS2"), "Output file exists, deleting old file: %s", filename.c_str());
      boost::filesystem::remove(filename);
    }
    // Open this file we want to write to
    outfile.open(filename.c_str());
    if (outfile.fail()) {
      RCLCPP_ERROR(rclcpp::get_logger("RecorderROS2"), "Unable to open output file: %s", filename.c_str());
